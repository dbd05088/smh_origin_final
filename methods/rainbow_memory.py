    ...
    def _train(
        ...
        for idx, data in enumerate(data_iterator):
            if len(data) == 2:
                ...
                print("stream", stream_data["image"].shape, "mem", mem_data["image"].shape, "pseudo", self.pseudo_loader['images'][idx].shape)
                ...
                self.pseudo_loader['labels'][idx] = self.pseudo_loader['labels'][idx].type(torch.LongTensor)
                self.pseudo_loader['labels'][idx] = self.pseudo_loader['labels'][idx].to(self.device)
                print("stream", stream_data["image"].shape, "pseudo", self.pseudo_loader['images'][idx].shape)
        ...
        for idx in range(last_idx+1, len(self.pseudo_loader['labels'])):
            self.pseudo_loader['images'][idx] = self.pseudo_loader['images'][idx].to(self.device)
            self.pseudo_loader['labels'][idx] = self.pseudo_loader['labels'][idx].type(torch.LongTensor)
            self.pseudo_loader['labels'][idx] = self.pseudo_loader['labels'][idx].to(self.device)
            print("pseudo shape", self.pseudo_loader['images'][idx].shape)
            l, c, d = self.update_model(self.pseudo_loader['images'][idx], self.pseudo_loader['labels'][idx], criterion, optimizer)
            total_loss += l
            correct += c
            num_data += d
        print("final_idx", len(self.pseudo_loader['labels']))

        if train_loader is not None:
            n_batches = len(train_loader) + (len(self.pseudo_loader['labels']) - last_idx)
        else:
